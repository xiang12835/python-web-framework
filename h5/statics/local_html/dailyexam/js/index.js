...
// 清除计时器，如果userId为空，则置入默认值，开始主流程
setTimeout(function () {
  clearInterval(getUserIdtimer);
  if (window.userId == '') {
    window.userId = !!getQueryString("user_id") ? getQueryString("user_id") : "201708071511258138685280";
  }
  main();
}, 1500);
// 主进程：
function main() {
  $("#app").show();
  var post_url = "https://api.platform.winlesson.com";
  var url = {
  ...
  window.app = new Vue({
    ...
    filters: {
      timefilter: function (value) {
        if (!value) return '--:--';
        if (value < 10) return '00:0' + value;else if (value < 60) return '00:' + value;else if (value < 60 * 60) {
          if (value % 60 < 10) {
      ...
      rankfilter: function (value) {
        ...
        if (value == 1) return '状元';else if (value == 2) return '榜眼';else if (value == 3) return '探花';else return value;
      },
      ...
      is_right_num: function (value) {
        if (!value || isNaN(parseInt(value))) return '-';else return value;
      }
    }
  });
  // VUE实例化：
