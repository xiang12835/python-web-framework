...
class XcXPayBase(object):
    ...
    def get_sign(self, param_map):
        ...
        sign_content = "{0}&key={1}".format(content, XcXPayConfig.KEY)
        #print sign_content
        smd5 = hashlib.md5()
        #print sign_content
...
class XcXPayDoPay(XcXPayBase):
    """
    微信下单接口
    """
    def __init__(self,  out_trade_no, total_fee, body='buy', subject='buy', payment_type="JSAPI",ip="",openid=''):
        
        self.url = "https://api.mch.weixin.qq.com/pay/unifiedorder"

        if settings.DEBUG:
            self.notify_url = "http://test.api.platform.winlesson.com/api/xcx/live/wepay/notice"
        else:
            self.notify_url = "http://api.msg.platform.winlesson.com/api/live/xcx/wepay/notice"
    ...
    def create_xml(self):
        """生成接口参数xml"""
        param_map = {
            'appid': XcXPayConfig.APPID,
            'mch_id': XcXPayConfig.MCHID,
