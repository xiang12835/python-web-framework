...
# def get_month_start_end_by_month(year,month):
#     import calendar
#     day_now = time.localtime()
#     day_begin = '%d-%02d-01 00:00:00' % (year, month)  # 月初肯定是1号
#     wday, monthRange = calendar.monthrange(year, month)  # 得到本月的天数 第一返回为月第一日为星期几（0-6）, 第二返回为此月天数
#     day_end = '%d-%02d-%02d 23:59:59' % (year, month, monthRange)

#     date_day_begin = datetime.datetime.strptime("%Y-%m-%d %H:%M:%S",day_begin)
#     date_day_end = datetime.datetime.strptime("%Y-%m-%d %H:%M:%S",day_end)
#     return date_day_begin,date_day_end
